...
$thumbsfolder=$_SERVER["DOCUMENT_ROOT"]."/thumbs";
...
function createthumb($name,$filename,$new_w,$new_h)
{
	$_imagefolder=$_SERVER["DOCUMENT_ROOT"]."/images/";
	$_thumbsfolder=$_SERVER["DOCUMENT_ROOT"]."/thumbs/";
	if (is_file($_thumbsfolder."tn_".$name)) {
    	return;
	}
	$system=explode(".",$name);	
	if (preg_match("/jpg|jpeg/",$system[1])){$src_img=imagecreatefromjpeg($_imagefolder.$name);}
	if (preg_match("/png/",$system[1])){$src_img=imagecreatefrompng($_imagefolder.$name);}
